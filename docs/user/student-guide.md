...
After clicking a site, you see the **Subnet View**. Each cloud shape is a subnet — a group of containers on the same network segment.

- **Click a subnet cloud** to see the individual containers inside it
- **Use the breadcrumb** at the top (e.g., `Home > Corporate Office`) to go back to the previous level

### LAN View — individual containers

After clicking a subnet cloud, you see the **LAN View** — the actual containers (computers, routers, servers, etc.).

Each container has a small colored dot:
- **Green dot** — the container is running and ready
- **Red dot** — the container is stopped or not yet deployed

---

## 3. Opening a Terminal

A terminal lets you type commands directly on a container — like opening a command prompt on that machine.

To open a terminal:

1. Navigate to the LAN View (click a site, then click a subnet cloud)
2. Find the container you want to connect to — it must have a **green dot**
3. Click on the container node
4. A terminal tab opens at the bottom of the screen

You're now connected to that container. Type your commands and press Enter.

---

## 4. Using the Terminal

**Multiple terminals:** You can open terminals to more than one container. Each container gets its own tab at the bottom of the screen. Click a tab to switch between containers.
...
**Refresh warning:** If you refresh or reload the page, all open terminals will close. You'll need to click the containers again to re-open them.
...
Clicking it opens the **Purdue Model View**, which shows how the containers in your lab are organized into security zones and levels. This is a read-only diagram — you can't make changes here.
...
- See container status (green/red dots)
- Open terminals to running containers
...
**Containers show red dots**
- The lab may not be deployed yet. Ask your instructor to deploy it.
- If some containers are red and others are green, those specific containers may have crashed — let your instructor know.

**Terminal stuck on "Connecting…"**
- The container must be green (running) before you can connect.
- Wait a few seconds for the container to finish starting, then try again.
- If it stays stuck, try clicking a different container first, then come back.
