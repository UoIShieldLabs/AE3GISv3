...
###
    ...
    return int(actual_rpm), int(current_vfd_freq)

def hardware_init():
    # global stuxnet_target
    # psm.set_var("QW1")
    psm.start()
...
def update_inputs():    
    ...
    update_rpm_true,stuxnet_freq = vfd_to_motor_rpm(vfd_freq_hz=target_freq_true, current_rpm=motor_rpm_true) 
    print(f"\n\nstuxnet target: {target_freq_true}\n\n")
    psm.set_var("IW0",update_rpm)
    psm.set_var("IW1", update_rpm_true)
...
def update_outputs():
    global stuxnet_freq
    if stuxnet_freq >= stuxnet_target:
        print(f"BEFORE: {psm.get_var("QW1")}")
        psm.set_var("QW1", int(0))
        print(f"AFTER: {psm.get_var("QW1")}")
        print("switch off")
    elif stuxnet_freq==0:
        psm.set_var("QW1",int(stuxnet_target))
        print("switch on")
